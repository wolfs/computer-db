 ...
 * @author wolfs
 */
object AngularApplication extends Controller {
  ...
  def logout(user: String) = AuthenticatedActionWithLogout(BodyParsers.parse.empty) { implicit request =>
    if (request.user == user) Ok else BadRequest("You cannot logout another user")
  }
