...
object Implicits {
  ...
  implicit def pkWrites[T](implicit fmt: Writes[T]): Writes[Pk[T]] = new Writes[Pk[T]] {
    def writes(o: Pk[T]) = o match {
      case Id(value) => fmt.writes(value)
      case NotAssigned => JsNull
    }
  }

  def pkFromOption(implicit r: Reads[Option[Long]]): Reads[Pk[Long]] = r.map( _.map(Id.apply _).getOrElse(NotAssigned))
  def pkToOption(implicit r: OWrites[Option[Long]]): OWrites[Pk[Long]] = r.contramap[Pk[Long]] {
  ...
  implicit val pairWrites: Writes[(Computer, Option[Company])] = (
    ...
    (__ \ "company").write[Option[Company]]
    tupled
    )
  implicit val pageWrites: Writes[Page[(Computer, Option[Company])]] = (
    (__ \ "items").write(Writes.traversableWrites[(Computer, Option[Company])]) and
    (__ \ "page").write[Int] and
    (__ \ "offset").write[Long] and
    (__ \ "total").write[Long]
    )(unlift(Page.unapply[(Computer, Option[Company])] _))
}
...
object Company {
  ...
  def list: Seq[Company] = DB.withConnection { implicit connection =>
