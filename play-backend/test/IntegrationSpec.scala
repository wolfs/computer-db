  ...
  "Application" should {
    ...
    "work from within a browser" in {
      running(TestServer(3333), HTMLUNIT) { browser =>
        ...
        browser.$("a.brand").first.getText must equalTo("Play 2.0 sample application — Computer database")
        browser.$("#main h1").first.getText must equalTo("574 computers found")
        ...
        browser.$(".search-query").text("Apple")
        browser.$("#searchsubmit").click()
        
        browser.$("#main h1").first.getText must equalTo("13 computers found")
        browser.$("a", withText("Apple II")).click()
        
        browser.$("#main h1").first.getText must equalTo("Edit computer")
